    ...
    match cmd {
        ...
        ConfigCommands::Get { key } => {
            match key.as_str() {
                ...
                _ => println!("Unknown config key: {}. Valid keys: device_name, relay_url, max_upload_mbps, dns_probe_ipv4, dns_probe_ipv6, quic_connect_timeout_ms, p2p_racing_timeout_ms, mdns_browse_timeout_ms", key),
            }
        }
            ...
            match key.as_str() {
                ...
                "max_upload_mbps" => {
                    if value.to_lowercase() == "none" {
                        ...
                        let mbps: u32 = value.parse()
                            .map_err(|e| anyhow::anyhow!("Invalid integer for max_upload_mbps: {}", e))?;
...
fn print_config_details(identity: &arc_core::crypto::identity::DeviceIdentity, config: &arc_core::ArcConfig) {
    ...
    println!("  max_upload_mbps:         {}", config.max_upload_mbps.map(|v| v.to_string()).unwrap_or_else(|| "unlimited".to_string()));
