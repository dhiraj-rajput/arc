...
async fn test_integration_pairing() {
    ...
    let sender_fut = run_pairing_sender(phrase, &ws_url, "sender-device");
    let receiver_fut = run_pairing_receiver(phrase, &ws_url, "receiver-device");
    ...
    assert_eq!(receiver_name, "receiver-device");
...
async fn test_integration_file_transfer() {
    ...
    let p_sender = run_pairing_sender(phrase, &ws_url, &config.device_name);
    let p_receiver = run_pairing_receiver(phrase, &ws_url, "test-peer");
