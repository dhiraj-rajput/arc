...
pub fn get_local_ips() -> Vec<IpAddr> {
    let (probe_ipv4, probe_ipv6) = match crate::storage::load_config() {
        ...
        Err(_) => ("8.8.8.8:80".to_string(), "[2001:4860:4860::8888]:80".to_string()),
    };
...
impl DiscoveryManager {
    ...
    pub fn register_service(&self, instance_name: &str, port: u16, device_id: &[u8; 32]) -> Result<(), anyhow::Error> {
        let local_ips = get_local_ips();
        let ip_to_use = local_ips.first().copied().unwrap_or(IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1)));
        ...
        info!("Registering mDNS service: {} with IP: {} and port: {}", fullname, ip_to_use, port);
    ...
    pub fn discover_device(&self, target_device_id: &[u8; 32], timeout: Duration) -> Option<SocketAddr> {
